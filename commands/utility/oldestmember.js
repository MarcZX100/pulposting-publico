...
module.exports = { //Lo exporta a el bot.js con el message.js
  name: "oldestmember", //Nombre de comando
  cooldown: 5, //Tiempo para volver a usar el comando (en segundos)
  aliases: ["oldest", "oldmember"], //Aliases para usarlo
  description: "This command displays the 10 oldest members at this guild!", //Descripción del comando
  ...
  exec: async (client, message, args) => { //Ejecutar con async para el await
    ...
    let memb = message.guild.members.cache.filter(m => !m.user.bot).sort((a,b) => a.joinedTimestamp - b.joinedTimestamp).first(10)
    const embed = new Discord.MessageEmbed()
    .setTitle(`Oldest Members from ${message.guild}`)
    .setColor(client.color)
memb.forEach(m => embed.addField(`${m} - ${m.user.tag}`, `The user ${m.user.username} with the ID ${m.user.id} was joined the\n <t:${(m.joinedTimestamp.toString()).slice(0, -3)}>. (<t:${(m.joinedTimestamp.toString()).slice(0, -3)}:R>)`)
            )
