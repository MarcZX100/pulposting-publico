...
module.exports = { //Lo exporta a el bot.js con el message.js
  ...
  exec: async (client, message, args) => { //Ejecutar con async para el await
    ...
    let url = await soyultro("hug")
    let user = message.mentions.users.first() ? `hugs ${message.mentions.users.first().tag}. aww` : `hugs me awww :heart:`;
