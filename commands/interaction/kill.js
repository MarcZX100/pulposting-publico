...
module.exports = {
  ...
  name: "kill", //Nombre de comando
  cooldown: 5, //Tiempo para volver a usar el comando (en segundos)
  aliases: [], //Aliases para usarlo
  description: "Kills someone! (Plsease noo)", //Descripción del comando
  ...
  exec: async (client, message, args) => {
    ...
    let url = (await message.mentions.users.first())
      ? soyultro("kill")
      : soyultro("die");
    let user = message.mentions.users.first()
      ? `has killed ${message.mentions.users.first().tag}!`
      : `please noo! You still have a lot to live  ;-;`;
