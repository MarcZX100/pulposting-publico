...
module.exports = {
  ...
  botpermissions: [],
	data: new SlashCommandBuilder()
		.setName('userinfo')
		.setDescription('Gets data from an user')
    .addUserOption(option => option.setName('mention').setDescription('Get user by mention'))
    .addStringOption(option => option.setName('id').setDescription('Get user by ID'))
  
  /*.addSubcommand(subcommand =>
		  subcommand
			  .setName('mention')
			  .setDescription('Get user by mention')
			  .addUserOption(option => option.setName('mention').setDescription('Get user by mention')))
      .addSubcommand(subcommand =>
		  subcommand
			  .setName('id')
			  .setDescription('Get user by ID')
        .addIntegerOption(option => option.setName('id').setDescription('Get user by ID')))
      .addSubcommand(subcommand =>
		  subcommand
			  .setName('random')
			  .setDescription('Get a random user from the guild'))
	...
	,async execute(client, interaction, author, guild) {
	  ...
	  booster: 'Server Booster <:boosting:907185190659584050>'
		...
		let persona;
		  ...
		  let mencion = interaction.options.getUser('mention');
		if(id) {
    if(id.length<18 || id.length>18 || isNaN(id)) return interaction.reply({content:"You must display an id that exists!", ephemeral: true})
    if(!guild.members.cache.get(id)) return interaction.reply({content:"That user is unavailable for me! Is him at this guild?", ephemeral: true})
	persona = guild.members.cache.get(id)
		}
		else if(mencion) {
    if(!guild.members.resolve(mencion)) return interaction.reply({content:"That user is unavailable for me! Is him at this guild?", ephemeral: true})
	persona = guild.members.resolve(mencion)
				}
		else{
		persona = interaction.member
		}
...
//!  let persona = args[0] === "random" ? º1message.guild.members.cache.random() : message.mentions.members.first()
        const presenc = {
        ...
        if (persona.id == guild.ownerId) userFlags.push("serverowner")
	...
	if (persona.premiumSince) userFlags.push("booster")
        const embed1 = new client.discord.MessageEmbed()
          ...
          .addField("**When entered the server?**", guild ? `<t:${(persona.joinedTimestamp.toString()).slice(0, -3)}>.\n(<t:${(persona.joinedTimestamp.toString()).slice(0, -3)}:R>)` : "DM Channel")
...
.setColor(client.color)
.setAuthor({name: interaction.user.tag, iconURL:interaction.user.displayAvatarURL({ format: 'png', dynamic: true, size: 2048 })})
